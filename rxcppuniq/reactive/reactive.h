 ...
 * Reactive streams are the mechanism with which programs communicate with
 * the environment: a database, the server, device sensors, network agents,
 * and so on.
 *
...
class Subscription {
 public:
  ...
  /**
   * Requests the stream to produce the number of specified events. There is
   * no guarantee that the given number is actually produced, but each
...
class Publisher : public UniqueObject<PublisherImpl<T>> {
 public:
   ...
   * size elements.
   */
  Publisher<std::vector<T>> Buffer(std::int32_t size) &&;
   ...
   *
   * TODO(team): this operator is currently not implemented.
   */
   ...
   */
  Publisher<T> Async(SharedObject<Worker> const &produce_on,
